...
class XMLGeneratorService
{
    ...
    private const XMLNS_CREDIT_NOTE = 'urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2';
    private const XMLNS_DEBIT_NOTE = 'urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2';
    ...
    public function generate(InvoiceData $invoice, SellerData $seller, ?BuyerData $buyer = null): string
    {
        ...
        $rootNamespace = match ($invoice->type) {
            InvoiceData::TYPE_CREDIT_NOTE => self::XMLNS_CREDIT_NOTE,
            InvoiceData::TYPE_DEBIT_NOTE => self::XMLNS_DEBIT_NOTE,
            default => self::XMLNS,
        };

        $this->root = $this->doc->createElementNS($rootNamespace, $rootName);
    ...
    {
        // Profile ID
        $profile = $invoice->needsClearance() ? 'clearance:1.0' : 'reporting:1.0';
        ...
        // Invoice Type Code
        $typeCodeElement = match ($invoice->type) {
            InvoiceData::TYPE_CREDIT_NOTE => 'cbc:CreditNoteTypeCode',
            InvoiceData::TYPE_DEBIT_NOTE => 'cbc:DebitNoteTypeCode',
            default => 'cbc:InvoiceTypeCode',
        };
        $typeCode = $this->createElement($typeCodeElement, $this->getDocumentTypeCode($invoice));
    ...
    /**
     * Get UBL document type code.
     */
    private function getDocumentTypeCode(InvoiceData $invoice): string
    {
        return match ($invoice->type) {
            InvoiceData::TYPE_CREDIT_NOTE => '381',
            InvoiceData::TYPE_DEBIT_NOTE => '383',
            default => '388',
        };
    }
    ...
    private function addInvoiceLines(InvoiceData $invoice): void
    {
        foreach ($invoice->lines as $index => $line) {
            ...
            // Quantity
            $quantityElement = match ($invoice->type) {
                InvoiceData::TYPE_CREDIT_NOTE => 'cbc:CreditedQuantity',
                InvoiceData::TYPE_DEBIT_NOTE => 'cbc:DebitedQuantity',
                default => 'cbc:InvoicedQuantity',
            };
            $invoicedQuantity = $this->createElement($quantityElement, (string) $line->quantity);
